...
export class CoreEvents {
  ...
  events() {
    $("body")
      ...
      .on("keypress", ".moneyonly", function (event) {
        ...
        if (
          keyCode > 0 &&
          event.which !== desAscii &&
          (event.which < 48 || event.which > 57)
        ) {
          if (
            "|8|".indexOf("|" + event.which + "|") === -1 &&
            "|37|39|".indexOf("|" + event.keyCode + "|") === -1
          ) {
            event.preventDefault();
          }
      ...
      .on("keypress", "[data-dateformat]", function (event) {
        ...
        if (event.which < 48 || event.which > 57) {
          if (event.char !== ayrac) event.preventDefault();
        }
      })
  ...
  lazy_load() {
    // Create a single IntersectionObserver instance for all lazy elements
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if ($(_target).is("picture")) {
              let source = $(_target).children();

              source.each(function () {
                if (
                  $(this).is("source") &&
                  $(this).attr("data-srcset") != undefined
                ) {
                  $(this)
              ...
              switch ($(_target).attr("data-lazy")) {
                case "number":
                  core_number_say(
                    $(_target),
                    0,
                    parseInt($(_target).attr("data-number"))
                  );
                  break;
                case "appointment":
            ...
            // Stop observing this element since it's been processed
            observer.unobserve(_target);
          }
        });
      },
      {
        // Add threshold and rootMargin for better performance
        threshold: 0.1,
        rootMargin: "50px",
      }
    );
    ...
    // Store the observer globally so it can be used later if needed
...
function core_number_say(obj, vl, max) {
  ...
  if (vl > max) {
    obj.html(
      max +
        (obj.attr("data-string") !== undefined ? obj.attr("data-string") : "")
    );
    return;
  }

  obj.html(
    vl + (obj.attr("data-string") !== undefined ? obj.attr("data-string") : "")
  );
